...
for c=1:numel(db_chunks)
  ...
  for i=1:numel(db_chunks{c})
    data = load(sprintf('feats/%s/facade_%04d.mat', dbname, db_chunks{c}(i)));
    if i==1
  ...
  for q=1:num_db
    query = load(sprintf('feats/%s/label_%04d.mat', dbname, q));
...
save(fullfile('results', dbname, sprintf('facades_%s_similarity.mat', disttype)), ...
     'similarity')
